...
class WechatPayService extends BasePayService
{
    /**
     * 授权信息
     * @var \App\Common\Model\User\UserAuth
     */
    protected $auth;
    ...
    public function __construct($terminal, $userId = null)
    {
        ...
        if ($userId !== null) {
            $this->auth = UserAuth::where(['user_id' => $userId, 'terminal' => $terminal])->firstOrFail();
        }
    }
    ...
    public function notify()
    {
        ...
        $server->handlePaid(function (Message $message) {
            if ($message['trade_state'] === 'SUCCESS') {
                ...
                switch ($attach) {
                    case 'recharge':
                        $order = RechargeOrder::where(['sn' => $message['out_trade_no']])->firstOrFail();
